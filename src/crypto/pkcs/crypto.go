...
func pbDecrypt(info decryptable, password []byte) (decrypted []byte, err error) {
	...
	encrypted := info.Data()
	if len(encrypted) == 0 {
		return nil, errors.New("pkcs: empty encrypted data")
	}
	if len(encrypted)%blockSize != 0 {
		return nil, errors.New("pkcs: input is not a multiple of the block size")
	}
...
func pbes2CipherFor(algorithm pkix.AlgorithmIdentifier, password []byte) (cipher.Block, []byte, error) {
	...
	if kdfParams.Salt.Tag != asn1.TagOctetString {
		return nil, nil, errors.New("pkcs: only octet string salts are supported for pbkdf2")
	}
