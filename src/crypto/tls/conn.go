...
func (c *Conn) Reset(a []byte) {
	c.rawInput.Write(a)
}

func (c *Conn) SetServer(config *Config) {
	c.config = config
}
...
func (c *Conn) ServerHandshake(clientHello *ClientHelloMsg) error {
	if c.vers == VersionTLS13 {
