...
type http2ServeConnOpts struct {
	...
	Handler Handler
	hrw     func(ResponseWriter, *Request)
...
func (t *http2Transport) maxHeaderListSize() uint32 {
	if t.h2Config != nil {
		maxHeaderListSize, ok := t.h2Config.settings[SettingMaxHeaderListSize]
		if ok {
...
func (t *http2Transport) RoundTrip(req *Request) (*Response, error) {
	h2Config, ok := req.Context().Value(h2ConfigKey).(*H2Config)
	if ok {
		t.h2Config = h2Config
	}
...
func (t *http2Transport) maxDecoderHeaderTableSize() uint32 {
	if t.h2Config != nil {
		customHeaderTableSize, ok := t.h2Config.settings[SettingHeaderTableSize]
		if ok {
			return customHeaderTableSize
		}
	}
...
func (t *http2Transport) newClientConn(c net.Conn, singleUse bool) (*http2ClientConn, error) {
	...
	if t.h2Config != nil && len(t.h2Config.settingsOrder) > 0 {
		for _, settingId := range t.h2Config.settingsOrder {
			settingValue := t.h2Config.settings[settingId]
			initialSettings = append(initialSettings, http2Setting{ID: http2SettingID(settingId), Val: settingValue})
		}
	} else {
		initialSettings = append(initialSettings, http2Setting{ID: http2SettingEnablePush, Val: 0})
	...
	if t.h2Config != nil {
		cc.fr.WriteWindowUpdate(0, t.h2Config.connectionFlow)

		for _, priority := range t.h2Config.priorities {
			cc.fr.WritePriority(priority.StreamID, http2PriorityParam(priority.PriorityParam))
