...
func (c *conn) readRequest(ctx context.Context) (w *response, err error) {
	...
	hosts := req.Header.Get("Host")
	isH2Upgrade := req.isH2Upgrade()
	if req.ProtoAtLeast(1, 1) && len(hosts) == 0 && !isH2Upgrade && req.Method != "CONNECT" {
		return nil, badRequestError("missing required Host header")
	}
	if len(hosts) == 1 && !httpguts.ValidHostHeader(hosts) {
...
func (cw *chunkWriter) writeHeader(p []byte) {
	...
	if !header.has("Date") && !header.has("date") {
...
func init() {
	h1s = &Server{ReadTimeout: 5 * time.Second}
	h2s = &http2Server{
...
type initALPNRequest struct {
	ctx context.Context
	c   *tls.Conn
	h   Handler // handler to invoke, http.DefaultServeMux if nil
}
