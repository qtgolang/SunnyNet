...
func (t *transferWriter) writeHeader(w Header, trace *httptrace.ClientTrace) error {
	...
	if t.shouldSendContentLength() {
		w.Set("Content-Length", strconv.FormatInt(t.ContentLength, 10))
		if trace != nil && trace.WroteHeaderField != nil {
	...
	if t.Trailer != nil {
		...
		if len(keys) > 0 {
			...
			w.Set("Trailer", strings.Join(keys, ","))
			if trace != nil && trace.WroteHeaderField != nil {
...
func (t *transferReader) parseTransferEncoding() error {
	raw := t.Header.GetArray("Transfer-Encoding")
	if len(raw) == 0 {
		return nil
	}
	t.Header.Del("Transfer-Encoding")
...
func fixLength(isResponse bool, status int, requestMethod string, header Header, chunked bool) (int64, error) {
	...
	contentLens := header.GetArray("Content-Length")
...
func shouldClose(major, minor int, header Header, removeCloseHeader bool) bool {
	...
	conv := header.GetArray("Connection")
...
func fixTrailer(header Header, chunked bool) (Header, error) {
	...
	for _, v := range vv {
		foreachHeaderElement(v, func(key string) {
			...
			if strings.EqualFold(key, "Transfer-Encoding") || strings.EqualFold(key, "Trailer") || strings.EqualFold(key, "Content-Length") {
				if err == nil {
